export const CLASSES = {
    BASE: 'autocomplete',
    CONTAINER: 'autocomplete-chbContainer',
...
export default class Autocomplete {
    ...
    selectOption(optionNode, callback) {
        ...
        optionNode = optionNode.id != "" ? optionNode : optionNode.parentNode;
    ...
    outputResults() {
        if (this.results.length > 0) {
            this.results.forEach((result) => {
                ...
                resultListItem.classList.add(CLASSES.RESULT.ITEM_CLASS);
                resultListItem.dataset.value = result.value;
                var dialCode = Countries.find(x => x.code.toLowerCase() === result.value.toLowerCase());
                resultListItem.dataset.prefix = dialCode ? dialCode.dial_code : '';
                resultListItem.dataset.content = result.label;
                ...
                resultListItem.appendChild(resultListItemDiv);
            ...
            noResultsItem.innerHTML = "<span style='font-size: 13px'>No results were found</span>";
            window.requestAnimationFrame(() => {
